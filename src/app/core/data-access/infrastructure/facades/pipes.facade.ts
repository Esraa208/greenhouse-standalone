...
export class PipesFacade {
  ...
  readonly #editingItem = signal<PipeRow | null>(null);
  readonly #deletingItem = signal<PipeRow | null>(null);
  ...
  readonly #selectItems = signal<PipeRow[]>([]);
  ...
  readonly filteredItems = computed<PipeRow[]>(() => {
    return this.#items();
  });
  ...
  #listQuery() {
    ...
    return {
      ...
      extra: {
        ...
        LayerId: f.layerId === 'all' ? '' : f.layerId,
      },
    };
  }
  ...
  create(dto: CreatePipeDto | UpdatePipeDto): void {
    this.closeModal();
    subscribeMutationWithResult({
      destroyRef: this.#destroyRef,
      mutation$: this.#repo.create(dto as CreatePipeDto),
      setLoading: (v) => this.#isLoading.set(v),
      setError: (msg) => this.#error.set(msg),
      onSuccess: (row) => {
  ...
  update(id: string, dto: CreatePipeDto | UpdatePipeDto): void {
  ...
  openDeleteModal(item: PipeRow): void {
